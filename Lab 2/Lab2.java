...
class Song {
    ...
    public int getEstEarnings() {
        return  title.length() * singer.getMinistry().getSongPartEst();  //Q2. Perform this calculation
    }
...
class Singer {
    ...
    public int sumEstValue() {
        ...
        for(Song countersong: songs)
        {
            sum += countersong.getEstEarnings();
        }
    ...
    public void tryToRegisterSong(Song song) {
        ...
        }else
        {
            ...
            if( studioExists(x) && canAfford(x) && x.isAvailable())
            {
                song.setStudio(x);
            	registeredSongs.add(song);
            	budget -= x.getCost();
