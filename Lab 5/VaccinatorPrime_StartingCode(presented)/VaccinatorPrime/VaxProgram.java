...
public class VaxProgram { 
    ...
    public void loadApproved(String afile)
    {
        ...
        try{
         ...
         apscan = new Scanner(new File (afile));   
    ...
    public ArrayList<VaccineBatch> loadVCBatches (String vcfile) throws IOException, ArrayIndexOutOfBoundsException    {
        ...
        vscan = new Scanner(new File(vcfile));

        while (vscan.hasNext()){

            String [] lines_vac = vscan.nextLine().split(" ");


            String name = lines_vac[0];
            int size = Integer.parseInt(lines_vac[1]);
            int preference = Integer.parseInt(lines_vac[2]);
            String cIndicators = lines_vac[3];

            VaccineBatch new_vac = new VaccineBatch(name, size, preference, cIndicators);
