...
public class VaxProgram { 
    ...
    public ArrayList<VaccineBatch> loadVCBatches (String vcfile) throws IOException, ArrayIndexOutOfBoundsException    {
        ...
        while(vscan.hasNext())
        {
            ...
            String [] vnextLine = vline.split(" ");

            int size = Integer.parseInt(vnextLine[1]);

            int preference = Integer.parseInt(vnextLine[2]);

            String cIndicators = "";

            if(vnextLine.length > 3){

                cIndicators = vnextLine[3];
            }

            VaccineBatch random = new VaccineBatch(vnextLine[0], size, preference, cIndicators);
    ...
    public void applyVaxes()
    {
        ...
        if (aplist.size()>0)
        {
            Collections.sort(aplist); //was a comment
            Collections.sort(vblist); //was a comment
            for (VaccineBatch vb:vblist)
            {
